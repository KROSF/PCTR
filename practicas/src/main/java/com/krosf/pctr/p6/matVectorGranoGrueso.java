...
public class matVectorGranoGrueso implements Runnable {
  ...
  private static final String[] opciones = { "      Matriz - Vector", "1. Insertar Matriz - Vector",
      "2. Matriz - Vector Aleatorios", "S. Salir" };
   ...
   */
  public matVectorGranoGrueso(int inicio, int fin) {
    this.inicio = inicio;
    this.fin = fin;
  }
  ...
  /**
   * Metodo para inicializar la matriz y el vector con datos de la entrada
   * estandar.
   * 
   * @param filas    de la matriz a inicializar.
   * @param columnas de la matriz a inicializar.
  ...
  public static void start(int filas, int columnas) {
    ...
    int coef = (int) Math.ceil((filas * 1.) / cores);
    int start = 0;
    int end = 0;
    boolean tasks = true;
    ...
    for (int i = 0; i < hilos.length; ++i) {
      ...
      if (tasks) {
        ...
        hilos[i] = new Thread(new matVectorGranoGrueso(start, end));
        hilos[i].start();
      }
    }
    try {
